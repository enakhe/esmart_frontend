...
const Hotel = () => {
  ...
  const [
    createHotel,
    {
      isLoading: creatHotelLoading,
      error,
      isError: createHotelError,
      isSuccess: createHotelSuccess,
    },
  ...
  useEffect(() => {
    if (isError) {
      toast.error("An error occurred: Not Authorized")
    }

    if (createHotelSuccess && data) {
      setOpenModal(false)
      refetch()
    }

    if (createHotelError && error) {
